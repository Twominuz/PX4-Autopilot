...
void AutoPreflightCheck::Run()
{
	...
	if(test_done){
		//start stamped time
		if ((_time_reset)&&(_time_reset_delay)){ //true and start if
			_previous_time = hrt_absolute_time();
			_time_reset = false;
			_time_reset_delay = false;
		}

        if (hrt_elapsed_time(& _previous_time)> 3_s) //wait 3 sec to start motor test
		{  //set every n second
            _time_reset = true;
			_do_motor_test = true;
		}
		...
		if (hrt_elapsed_time(& _previous_time_2)> 5_s)
		{  //set every n second
